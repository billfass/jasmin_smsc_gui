...
@action('web/dlr/checking', method=['GET','POST'])
@action.uses(db, session, auth, flash)
def web_checking():
    data = request.POST
    import uuid
    ...
    try:
        ...
        r = requests.post("https://fastermessage.com/app/sms/message/dlr/"+data['messageId'], data=dict(data), headers={})
        r.close()
        if r.status_code == 200:
            return 'ACK/Jasmin'
...
@action('web/batchdlr/checking', method=['GET','POST'])
@action.uses(db, session, auth, flash)
def web_batch_checking():
    data = request.POST
    callback = None
    import uuid
